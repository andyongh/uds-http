...
1. **Cross-Platform Event Loop (`libuv`)**: Natively supports both Linux (`epoll`) and macOS (`kqueue`) without source changes, exceeding the concurrency threshold.
2. **Parsing Efficiency**: Integrated `llhttp` for pipelined HTTP parsing, and `yyjson` for blistering fast JSON parsing and serialization.
3. **Concurrency (<1000)**: The main [accept](file:///Users/andy/Downloads/vibe_coding/uds-http-server/src/http_server.c#211-235) loop pauses after 1000 concurrent active connections.
4. **Drop Bucket Flow Control**: Uses a background `uv_timer_t` algorithm to enforce an RPS limit via a Token Bucket system. Exceeding requests instantly receive `429 Too Many Requests`.
...
7. **Keep-Alive**: Fully supports keep-alive connections which was crucial for reaching benchmark maximums.
...
- **Libraries Vendored**: `llhttp` (generated `api.c`, `llhttp.c`, `http.c`), `yyjson`, `lz4`.
